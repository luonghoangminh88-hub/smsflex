  ...
  const countryStats = useMemo(() => {
    ...
    return filteredCountries
      .map((country) => {
        ...
        // TODO: Replace with real success rate from database
    ...
    <div className="flex flex-col lg:flex-row gap-4 lg:gap-6 min-h-[600px]">
      <div className="lg:hidden mb-4">
        ...
        >
          ...
          {selectedService ? `${selectedService.name} - Chọn dịch vụ khác` : "Chọn dịch vụ"}
        </Button>
      </div>
                    ...
                    <div className="space-y-1.5 max-h-[500px] overflow-y-auto pr-2">
                      {filteredServices.map((service) => {
                        ...
                        return (
                          <div
                            key={service.id}
                            className={`flex items-center gap-2 p-2.5 rounded-lg cursor-pointer transition-all hover:bg-accent group min-h-[48px] ${
                              isSelected
                ...
                <div className="space-y-1.5 max-h-[400px] overflow-y-auto pr-2">
                  {filteredServices.map((service) => {
                    ...
                    return (
                      <div
                        key={service.id}
                        className={`flex items-center gap-2.5 p-3 rounded-lg cursor-pointer transition-all hover:bg-accent group min-h-[52px] ${
                          isSelected
                            ? "bg-blue-50 dark:bg-blue-950/20 border border-blue-200 dark:border-blue-800"
                        ...
                        onClick={() => handleServiceSelect(service)}
                      >
                        <div className="w-8 h-8 rounded-full bg-white dark:bg-gray-800 flex items-center justify-center overflow-hidden flex-shrink-0 border">
                          {!hasImageError ? (
                            <Image
                              src={getServiceLogo(service.name) || service.icon_url || "/placeholder.svg"}
                              alt={service.name}
            ...
            {!selectedService ? (
              <div className="flex flex-col items-center justify-center py-12 sm:py-16 text-center space-y-4 px-4">
                <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center">
                  <Search className="h-7 w-7 sm:h-8 sm:w-8 text-blue-600 dark:text-blue-400" />
                </div>
                <div>
              ...
              <div className="space-y-4 sm:space-y-6">
                ...
                <div className="flex items-center gap-3 pb-3 sm:pb-4 border-b">
                  {(() => {
                    ...
                    return (
                      <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-white dark:bg-gray-800 flex items-center justify-center overflow-hidden border-2">
                        {!hasError ? (
                          <Image
                          ...
                          <FallbackIcon className="h-6 w-6 sm:h-7 sm:w-7 text-primary" />
                        )}
                      </div>
                    )
                  })()}
                  <div className="flex-1 min-w-0">
                    <h3 className="font-bold text-base sm:text-lg truncate">{selectedService.name}</h3>
                ...
                <div className="space-y-2">
                  <div className="flex flex-wrap items-center gap-2 mb-3">
                    ...
                    <Badge variant="secondary" className="text-xs hidden sm:flex items-center ml-auto">
                  ...
                  <div className="space-y-2 max-h-[400px] overflow-y-auto pr-2">
                    {countryStats.map(({ country, price, stock, successRate }) => {
                      ...
                      return (
                        <div
                          key={country.id}
                          className={`border rounded-lg p-3 sm:p-4 cursor-pointer transition-all hover:shadow-md min-h-[56px] ${
                            isSelected
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <div className="text-2xl sm:text-3xl flex-shrink-0">{getCountryFlag(country.code)}</div>
                            ...
                            <div className="flex-1 min-w-0 space-y-2">
                              <div className="flex items-center justify-between gap-2">
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="text-xs text-muted-foreground lg:hidden">
                                    {stock.toLocaleString(locale === "vi" ? "vi-VN" : "en-US")} số
                                  </div>
                                </div>
                                <Badge
                              ...
                              <div className="hidden lg:block">
                                <div className="flex items-center justify-between text-xs text-muted-foreground mb-1">
                                  <span>{t("service.successRate")}</span>
                              ...
                              {/* Price Bar - Shown on both mobile and desktop */}
                              <div className="bg-gray-100 dark:bg-gray-800 rounded-full h-7 sm:h-8 overflow-hidden relative">
                                <div
                                  className="h-full bg-gradient-to-r from-green-500 to-emerald-500 flex items-center justify-center text-xs sm:text-sm font-semibold text-white transition-all"
                                  style={{ width: `${Math.max(priceBarWidth, 20)}%` }}
                                >
                                  {formatVND(price)}
                                </div>
                              </div>
                            ...
                            <Button
                              ...
                              className="flex-shrink-0 h-11 w-11 sm:h-10 sm:w-10 p-0"
                              onClick={(e) => {
                                e.stopPropagation()
                ...
                {selectedCountry && selectedPrice && (
                  <div className="mt-4 sm:mt-6 p-4 sm:p-5 border-2 border-blue-200 dark:border-blue-800 rounded-xl bg-blue-50 dark:bg-blue-950/20 space-y-3">
