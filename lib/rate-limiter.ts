...
export async function checkRateLimit(
  ...
  try {
    ...
    const ipAddress = safeIdentifier.includes(":") ? safeIdentifier.split(":")[1] : safeIdentifier
...
export async function applyRateLimit(identifier: string, action: keyof typeof DEFAULT_LIMITS) {
  const result = await checkRateLimit(identifier, action)

  if (!result.success) {
    throw new Error(`Yêu cầu quá nhanh. Vui lòng thử lại lúc ${new Date(result.reset).toLocaleTimeString()}`)
  }
